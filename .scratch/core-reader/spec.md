...
- **Launcher** (glossary term): `run.bat` at the repo root, downloadable as a single file. Checks for Node.js (`where node`); if missing, asks the user (`choice /C YN`) before attempting a `winget install` of the Node.js LTS package, falling back to opening `https://nodejs.org` if winget is unavailable, the install fails, or the user declines — never installs anything without asking first. If Node.js is present (or just installed), runs `npx github:Babyferret/chatdiwa` — no separate update logic, since `npx`/`npm install` against an unpinned `github:` spec was verified (empirically, against a throwaway local git repo) to always re-resolve to the latest commit on install, with no manual cache-busting needed.
  ...
  - **Verified failure mode, fixed (superseding the above)**: a real external user's run failed with `ENOENT spawn git` — the `github:` npm spec shells out to a locally installed Git, an undocumented dependency beyond Node.js. Underneath that, the repo had also been private since creation despite intending otherwise, which every prior successful test in this project had masked (silently authenticated via the developer's own `gh auth setup-git` credentials). Both fixed: the repo is now public, and `run.bat`/the README's dev command use `npx https://github.com/Babyferret/chatdiwa/archive/refs/heads/master.tar.gz` instead — plain HTTPS + tarball extraction, no Git involved, same "always reflects current `master`" property. See `docs/adr/0003-distribute-via-tarball-url-not-github-spec.md`.

### Amendment: Tunnel for TikTok LIVE Studio (see ADR-0004)

TikTok LIVE Studio's Link source (its Browser-Source equivalent) rejects `localhost` URLs outright, unlike OBS. Added an optional, off-by-default Tunnel (`config.tunnelEnabled`): when turned on from the Control Panel, ChatDiWa downloads `cloudflared.exe` on first use (into `.chatdiwa-bin/`) and manages a Cloudflare Quick Tunnel process itself, surfacing the resulting public URL (which changes on every restart) live in the Control Panel with a copy button — no manual tool installation or terminal use required. `POST /api/config` starts/stops the Tunnel when `tunnelEnabled` actually changes value; `GET /api/tunnel-status` and a `{type: "tunnel"}` WebSocket broadcast keep the UI in sync. ngrok's free tier was ruled out: its interstitial "Visit Site" warning page can't be clicked through by TikTok's fetching mechanism. Verified end-to-end for real: fresh binary download, tunnel start, URL extraction, and both HTTP and WebSocket traffic proxied correctly through the resulting `trycloudflare.com` URL.
